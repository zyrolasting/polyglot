...
@title[#:tag "multiple-workflows"]{Tutorial: Using Multiple Workflows}
By the time you read this you should have used a Polyglot workflow for long
enough to feel somewhat restrained by it. Even if a workflow does some things
well, you will eventually tire of accomodating it for what it does @italic{not}
do well. So what do you do with existing code if you use a better, but
...
@section{Use @tt{.literal} to Mark External Dependencies} You cannot directly
depend on an asset used in a different workflow because the dependencies can
form cycles and prevent builds from terminating. @(base-workflow) supports @tt{.literal}
links to request resources relative to the project's distribution. This frees
one workflow to fulfill resources for another without conflicting builds.
...
Let's create two Polyglot projects in a directory.  One follows
@(imperative-workflow), one follows @(functional-workflow).
...
@(functional-workflow), but they can still evolve independently. We'll
